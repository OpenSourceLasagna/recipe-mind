...
def upgrade() -> None:
    ...
    op.drop_constraint(
        "recipe_ingredients_category_id_fkey", "recipe_ingredients", type_="foreignkey"
    )
    op.drop_constraint(
        "recipe_ingredients_recipe_id_fkey", "recipe_ingredients", type_="foreignkey"
    )
    ...
    op.drop_constraint(
        "ingredient_categories_pkey", "ingredient_categories", type_="primary"
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_diet_tags_recipe_id_recipes", "diet_tags", type_="foreignkey"
    )
    ...
    op.drop_constraint(
        "pk_ingredient_categories", "ingredient_categories", type_="primary"
    )
    ...
    op.create_check_constraint(
        "recipes_spice_level_check", "recipes", "spice_level >= 1 AND spice_level <= 5"
    )

    # 6. Re-create original implicit Foreign Keys
    op.create_foreign_key(
        "recipe_ingredients_category_id_fkey",
        "recipe_ingredients",
        "ingredient_categories",
        ["category_id"],
        ["id"],
