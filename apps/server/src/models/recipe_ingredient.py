...
class RecipeIngredient(SQLModel, table=True):
    ...
    normalized_name: str | None = Field(nullable=True, index=True, default=None)

    quantity: float = Field(
        sa_column=Column(Float, default=1.0, server_default=text("1.0"), nullable=False)
    )
    unit: str = Field(default="", sa_column_kwargs={"server_default": "''"})
    ...
    recipe: "Recipe" = Relationship(
        back_populates="ingredients", sa_relationship_kwargs={"lazy": "selectin"}
    )
