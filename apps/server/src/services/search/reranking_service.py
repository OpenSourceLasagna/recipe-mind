...
class RerankingService:
    def __init__(
        self,
        recipe_preprocessor: RecipePreprocessorService,
        model_name: str = "cross-encoder/ms-marco-MiniLM-L-6-v2",
        model_base_path: str = "./models",
        below_best_match_threshold: float = 4.0,
        general_acceptance_threshold: float = -7.0,
    ):
        ...
        self._general_acceptance_threshold = general_acceptance_threshold

    def rerank(self, scored: list[ScoredRecipe], query: str) -> list[Recipe]:
        if not scored:
            return []

        recipe_corpus = [self._recipe_preprocessor.to_markdown(r.recipe) for r in scored]
        ranks = self._model.rank(query, recipe_corpus)  # type: ignore

        results: list[Recipe] = []
        best_score = max(cast(float, rank["score"]) for rank in ranks)
        if best_score < self._general_acceptance_threshold:
            return []
