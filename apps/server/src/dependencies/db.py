...
async def get_db_session(
    _request: Request, current_user_id: CurrentUserID
) -> AsyncGenerator[AsyncSession]:
    async_session = AsyncSessionLocal()
    try:
        set_claim_statement = text(
            "SELECT set_config('request.jwt.claim.sub', :user_id, true);"
        )
        await super(AsyncSession, async_session).execute(
            set_claim_statement, {"user_id": str(current_user_id)}
        )
        yield async_session
    finally:
...
async def get_recipe_ingredient_repository(
    db: AsyncSession = Depends(get_db_session),
) -> RecipeIngredientRepository:
    return RecipeIngredientRepository(a_session=db)


async def get_ingredient_category_repository(
    db: AsyncSession = Depends(get_db_session),
) -> IngredientCategoryRepository:
    return IngredientCategoryRepository(a_session=db)


async def get_query_cache_repository(
    db: AsyncSession = Depends(get_db_session),
...
RecipeIngredientRepo = Annotated[
    RecipeIngredientRepository, Depends(get_recipe_ingredient_repository)
]
IngredientCategoryRepo = Annotated[
    IngredientCategoryRepository, Depends(get_ingredient_category_repository)
]
